...
if (argv.includes('--json')) {
  ...
  lines.push('`→N` = N other items name this task in their `needs:` — planning it unblocks them.')
  lines.push('')
  if (eligible.length === 0) {
    lines.push('  (none — every backlog task is claimed, blocked, or conflicting)')
  }
  for (const row of chunk(eligible, 8)) {
    lines.push('  ' + row.map((id) => eligibleLabel(id).padEnd(8)).join('').trimEnd())
  }
  ...
  lines.push('priorities are close, a higher `→N` unblocks more of the backlog. --claim <id> first.')
