...
const epicSiblings = epic
  ? all.filter((x) => x.epicHeading && x.epicHeading.line === epic.line)
  : []
const initiativeSiblings = all.filter(
  (x) => x.initiativeHeading && x.initiativeHeading.line === initiative.line,
)
...
const span = removeStart === removeEnd ? `line ${removeStart}` : `lines ${removeStart}–${removeEnd}`
const out: string[] = []
...
if (scope === 'item') {
  ...
  out.push(`   Delete ${span} — ∆${id} was the only item in initiative “${initiative.text}”,`)
...
if (dryRun) {
  out.push(`   Would release ∆${id}${wasClaimed ? '.' : ' (not currently claimed — would be a no-op).'}`)
} else {
  out.push(`   Released ∆${id}${wasClaimed ? '.' : ' (was not claimed — release recorded anyway).'}`)
}
out.push('')
out.push(
  dryRun
